...
describe("MoneySiren release installer", () => {
  it("downloads selected web and HUD release assets with checksum verification", async () => {
    ...
    const hudInstallerAsset = "MoneySiren.Tray_0.1.0-alpha.42_x64-setup.exe";
    const hudAsset = "MoneySiren.Tray_0.1.0-alpha.42_x64-portable.exe";
    ...
    expect(requests).toContain("https://api.github.com/repos/ztwz11/moneysiren/releases/tags/v0.1.0-alpha.42");
  ...
  it("prefers independently configured Windows signer thumbprints over release metadata", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "moneysiren-release-"));
    const installDir = join(cwd, "installed");
    const hudAsset = "MoneySiren_0.1.0-alpha.42_x64-setup.exe";
  ...
  it("fails unsigned Windows HUD alpha installs when explicitly disabled", async () => {
    ...
    await expect(installReleaseAssets({
      ...
      "Release asset signature verification failed for MoneySiren_0.1.0-alpha.42_x64-setup.exe: missing-signature-metadata",
    );
  ...
  it("resolves platform-specific default install directories", () => {
    ...
    expect(resolveReleaseInstallDir({
      ...
      platform: "win32",
      tag: "v0.1.0-alpha.42",
    })).toBe("C:\\Users\\tester\\AppData\\Roaming\\MoneySiren\\releases\\v0.1.0-alpha.42");
    ...
    expect(resolveReleaseInstallDir({
      ...
      platform: "darwin",
      tag: "v0.1.0-alpha.42",
    })).toBe("/Users/tester/Library/Application Support/MoneySiren/releases/v0.1.0-alpha.42");
  });
});
...
function releaseAsset(name: string) {
  return {
    name,
    browser_download_url: `https://github.com/ztwz11/moneysiren/releases/download/v0.1.0-alpha.42/${name}`,
    size: 1,
  };
}
