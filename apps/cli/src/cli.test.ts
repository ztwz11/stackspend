...
describe("MoneySiren CLI", () => {
  ...
  it("prints the pinned install release default separately from the CLI package version", async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(CLI_VERSION).toBe("0.1.0-alpha.45");
    expect(stdout).toContain("Release default: ztwz11/moneysiren@v0.1.0-alpha.45.");
  });
  ...
  it("installs selected release assets from GitHub Releases without storing secrets", async () => {
    ...
    const profilePath = join(cwd, "install-profile.json");
    const webAsset = "moneysiren-web-runtime-v0.1.0-alpha.45.tar.gz";
    ...
    const result = await runCli(
      ["install", "--all", "--dir", installDir, "--tag", "v0.1.0-alpha.45"],
      {
        ...testContext(cwd, {
          MONEYSIREN_INSTALL_PROFILE_PATH: profilePath,
        ...
        fetch: async (input) => {
          ...
          if (url.endsWith("/repos/ztwz11/moneysiren/releases/tags/v0.1.0-alpha.45")) {
            return Response.json({
              html_url: "https://github.com/ztwz11/moneysiren/releases/tag/v0.1.0-alpha.45",
              assets: [
                {
                  name: webAsset,
                  browser_download_url: `https://github.com/ztwz11/moneysiren/releases/download/v0.1.0-alpha.45/${webAsset}`,
                },
                {
                  name: hudAsset,
                  browser_download_url: `https://github.com/ztwz11/moneysiren/releases/download/v0.1.0-alpha.45/${hudAsset}`,
                },
                {
                  name: "moneysiren-web-runtime-SHA256SUMS.txt",
                  browser_download_url: "https://github.com/ztwz11/moneysiren/releases/download/v0.1.0-alpha.45/moneysiren-web-runtime-SHA256SUMS.txt",
                },
              ],
            });
          }
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout.join("\n")).toContain("Release: ztwz11/moneysiren@v0.1.0-alpha.45");
  ...
  it("does not persist the install profile when release asset installation fails", async () => {
    ...
    const installDir = join(cwd, "release-install");
    const profilePath = join(cwd, "install-profile.json");
    const hudAsset = "MoneySiren.Tray_0.1.0-alpha.45_x64-setup.exe";
    const hudBytes = Buffer.from("fake hud binary");

    const result = await runCli(
      ["install", "--hud", "--dir", installDir, "--tag", "v0.1.0-alpha.45"],
      {
        ...testContext(cwd, {
          MONEYSIREN_INSTALL_PROFILE_PATH: profilePath,
            ...
            return Response.json({
              ...
              assets: [
                ...
                },
                {
                  ...
                  browser_download_url: "https://github.com/ztwz11/moneysiren/releases/download/v0.1.0-alpha.45/moneysiren-tray-windows-SHA256SUMS.txt",
                },
              ],
            });
          }
