...
export function redactProviderPayload(value: unknown): RedactedJsonValue {
  ...
  for (const [key, nestedValue] of Object.entries(value)) {
    const keyMarker = markerForSensitiveKey(key);
...
function markerForSensitiveKey(key: string): string | undefined {
  ...
  if (normalizedKey.includes("token") || normalizedKey.includes("secret") || normalizedKey.includes("apikey")) {
    return "[REDACTED:token]";
  }

  if (normalizedKey.includes("accountid")) {
    return "[REDACTED:account_id]";
  }

  if (normalizedKey.includes("projectid")) {
    return "[REDACTED:project_id]";
  }

  if (normalizedKey.includes("invoiceid")) {
